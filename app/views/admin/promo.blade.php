...
@section('content')
	...
	<div class="main-content">
		...
		<form action="{{ URL::route('promo-post') }}" class="form-horizontal has-success has-feedback" role="form" method="post">
		<div class="form-group">
		    <label for="inputEmail3" class="col-sm-2 control-label">Promo Name</label>
		    <div class="col-sm-10">
		      <input type="text" name="name" class="form-control" id="name"{{ (Input::old('name'))? ' value="' . Input::old('name') . '"' : '' }}>
		      @if($errors->has('name'))
					{{ $errors->first('name') }}
				@endif
		    </div>
		  ...
		  <div class="form-group">
		    ...
		    <div class="col-sm-10">
		      <input type="text" name="total" class="form-control" id="inputEmail3"{{ (Input::old('total'))? ' value="' . Input::old('total') . '"' : '' }}>
		      @if($errors->has('total'))
					{{ $errors->first('total') }}
				@endif
		    </div>
